...
export default async function HomeStagingPage() {
  ...
  return <>
    ...
    <section className="mx-auto max-w-[1400px] px-5 py-16 md:px-10"><h2 className="font-display text-3xl">{en ? "A first impression that makes a difference." : "Une première impression qui fait la différence."}</h2><p className="mt-4 max-w-3xl text-secondary">{en ? "Our goal: help you sell or rent sooner and support the value of your property through thoughtful presentation." : "Notre objectif : vous aider à vendre ou louer plus vite et à défendre la valeur de votre bien grâce à une présentation soignée."}</p><div className="mt-10 grid gap-6 md:grid-cols-3">{services.map(([title, description], i) => <article key={title} className="rounded-[20px] border border-sand p-7"><p className="label-xs text-accent">0{i + 1}</p><h3 className="mt-4 font-display text-2xl">{title}</h3><p className="mt-3 leading-relaxed text-secondary">{description}</p></article>)}</div></section>
