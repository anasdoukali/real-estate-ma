  ...
  return <main className="relative isolate min-h-svh bg-charcoal text-white">
    ...
    <div className="mx-auto grid min-h-svh max-w-[1500px] items-center gap-12 px-6 py-14 md:px-12 lg:grid-cols-[1.2fr_1fr] lg:gap-20">
      <div>
        ...
        <p className="mt-8 max-w-xl whitespace-pre-line text-base leading-relaxed text-white/90">{maintenance.description}</p>
        <div className="mt-9 flex flex-wrap gap-4">
          {agency.email && <a href={`mailto:${agency.email}`} className="rounded-[20px] bg-white px-6 py-4 text-sm font-semibold text-charcoal hover:bg-sand">Contactez-nous</a>}
          {agency.phone && <a href={`tel:${agency.phone.replace(/[^+0-9]/g, "")}`} className="rounded-[20px] border border-white/60 px-6 py-4 text-sm hover:bg-white/10">{agency.phone}</a>}
        </div>
      </div>
      <div className="grid grid-cols-2 items-center gap-4" aria-label="Immobilier à Marrakech">
        <div className="relative aspect-[3/4] overflow-hidden rounded-[20px]"><Image src="https://images.pexels.com/photos/9730025/pexels-photo-9730025.jpeg?auto=compress&cs=tinysrgb&w=900" alt="Villa et espace extérieur" fill sizes="(max-width: 1024px) 45vw, 22vw" className="object-cover"/></div>
        <div className="relative mt-16 aspect-[3/4] overflow-hidden rounded-[20px]"><Image src="https://images.pexels.com/photos/7005300/pexels-photo-7005300.jpeg?auto=compress&cs=tinysrgb&w=900" alt="Intérieur lumineux et accueillant" fill sizes="(max-width: 1024px) 45vw, 22vw" className="object-cover"/></div>
      </div>
    </div>
  </main>;
}
