...
const field = "mt-2 h-12 w-full rounded-[20px] border border-sand bg-white px-4 text-[14px] outline-none focus:border-champagne";
...
export default function ServiceForm({ lang, service, neighborhoods }: {
  ...
  if (state === "done") return <div role="status" className="rounded-[20px] border border-champagne bg-white p-10"><h2 className="font-display text-3xl">{en ? "Request received." : "Demande reçue."}</h2><p className="mt-4 text-muted">{en ? "An advisor will contact you to discuss your project." : "Un conseiller vous recontactera pour étudier votre projet."}</p></div>;
  return <form onSubmit={submit} className="rounded-[20px] border border-sand bg-warm p-6 md:p-10">
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <label className="text-sm">{en ? "Desired budget" : "Budget souhaité"} (MAD{!staging && transaction === "rent" ? en ? " / month" : " / mois" : ""})<input name="budget" type="number" min="0.01" max="1000000000000" step="0.01" required className={field}/><span className="mt-2 block text-xs text-muted">{staging ? en ? "Budget for staging services." : "Budget dédié à la prestation Home Staging." : en ? "Your target sale price or monthly rent." : "Prix de vente ou loyer mensuel souhaité."}</span></label>
      ...
      <label className="text-sm sm:col-span-2">{en ? "Tell us about your project (optional)" : "Votre projet (facultatif)"}<textarea name="message" maxLength={5000} className="mt-2 min-h-32 w-full rounded-[20px] border border-sand bg-white p-4 outline-none focus:border-champagne"/></label>
    </div>
    <button disabled={state === "loading"} className="label-xs mt-6 rounded-[20px] bg-charcoal px-7 py-4 text-white transition-colors hover:bg-champagne disabled:opacity-60">{state === "loading" ? en ? "Sending…" : "Envoi…" : staging ? en ? "Request a quote" : "Demander un devis" : en ? "List your property" : "Confiez-nous votre bien"}</button>
