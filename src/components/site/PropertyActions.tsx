...
export default function PropertyActions({ id, lang }: { id: number; lang: Lang }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-6 text-[13px]">
      <button onClick={() => toggle(id)} className={`flex items-center gap-2 ${isFav ? "text-accent" : "hover:text-accent"}`}>
      ...
      <button
        ...
        className={`flex items-center gap-2 ${cmp.includes(id) ? "text-accent" : "hover:text-accent"}`}
      >
        ⇄ {en ? "Compare" : "Comparer"}
      </button>
      <button onClick={share} className="hover:text-accent">
        {en ? "Share" : "Partager"}
      </button>
      <button onClick={() => window.print()} className="hover:text-accent">
        {en ? "Print" : "Imprimer"}
      </button>
    </div>
  );
